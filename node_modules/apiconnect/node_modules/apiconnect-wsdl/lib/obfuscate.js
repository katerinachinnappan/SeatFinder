...
@return promise
*/
function obfuscate(inZip, outZip, outMap, inMap, options) {
    options = options || {};
    let def = q.defer();
    let req = options.req;
    ...
    try {
        ...
        if (isZip) {
            JSZip.loadAsync(inContent).then(function(out) {
                obfuscateContent(inContent, options).then(function(data) {
                    let result;
                    // Create a new zip file and add the new obfuscated files
                    let outContent = new JSZip();
                    for (let i = 0; i < data.length; i++) {
                        outContent.file(data[i].filename, data[i].content);
                    }
                    // Generate and write the files
                    outContent.generateAsync({
                        type: 'nodebuffer',
                        compression: 'DEFLATE'
...
*/
function obfuscateContent(inContent, options) {
    options = options || {};
    let req = options.req;
    var def = q.defer();
    var fileDef = q.defer();
    var fetchDef = parseUtils.getFileContent(inContent, inContent, null, null, req);
    fetchDef.then(function(rawContent) {
                                    ...
                                    readStream.on('end', function() {
                                        ...
                                        if (entry.fileName.indexOf('.wsdl', nameLen - 5) != -1 || entry.fileName.indexOf('.xsd', nameLen - 4) != -1) {
                                            try {
                                                ...
                                                fileContent = u.fixTags(fileContent);
                                                fileContent = parseUtils.protectDocumentation(fileContent);

                                                // Load the DOM, files that cannot be loaded will not be included
                                                // in the output zip.
                                                var DOMParser = require('xmldom').DOMParser;
                                                let wsdlDoc = new DOMParser().parseFromString(fileContent, 'text/xml');
                                                if (wsdlDoc) {
                                                    d.removeDTD(wsdlDoc);
                                                    ...
                                                    try {
                                                        let outputDoc = new DOMParser().parseFromString(fileContent, 'text/xml');
                                                        let map = d.getNamesMap(outputDoc);
                                                        checkMap(map, entry.fileName);
                                                    } catch (e) {
                                                        // Should throw term error...but still debugging
                                                        console.log('Could not parse file ' + entry.fileName + ' ' + e);
                                                    }
                                                    if (!isUTF8(encoding)) {
                                                        fileContent = parseUtils.encode(fileContent, encoding);
                                                    }
                                                    // File information is pushed, which will cause it to be included
                                                    // in the zip.
                                                    var file = {
                                                        filename: obfuscatePath(entry.fileName),
                                                        content: fileContent
                                                    };
                                                    data.push(file);
                                                }
        ...
        fileDef.promise.then(function(result) {
            def.resolve(result);
        }, function(e) {
            def.reject(e);
        });
...
function checkMap(map, fileName) {
    ...
    // These are the nodes and attributes that should have been removed.
    let scrubbedNodes = [ 'appinfo', '#comment' ];
    let scrubbedAttrs = [ 'id', 'source', 'xpath', 'refer' ];
