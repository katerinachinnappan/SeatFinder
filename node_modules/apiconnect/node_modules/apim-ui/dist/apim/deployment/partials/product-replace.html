...
********************************************************** {COPYRIGHT-END} **/
-->
<h2 class="bx--modal-content__heading" role="banner" aria-label="{{::'product_manage_replace' | translate}}">
  <div class="bx--form__legend floatLeft">{{::'product_manage_replace' | translate}}</div>
...
<div class="bx--modal-content webapi bx--replace-existing" data-ng-switch="currentPage">
  <div data-ng-switch-default>
    <h5 style="margin-bottom: 15px;">{{::'product_manage_replace_description' | translate}}</h5>
    <ol class="bx--list--ordered">
      <li class="bx--list__item">{{::'product_manage_replace_action1' | translate}}</li>
      <li class="bx--list__item">{{::'product_manage_replace_action2' | translate}}</li>
      <li class="bx--list__item">{{::'product_manage_replace_action3' | translate}}</li>
      <li class="bx--list__item">{{::'product_manage_replace_action4' | translate}}</li>
    </ol>
    <h5 style="margin-bottom: 15px;">{{::'product_manage_select_to_replace' | translate}}</h5>
    ...
    <div class="primaryBorder" style="margin-bottom: 15px;">
      ...
      <ul class="simpleList productVersions cardContainer">
        <li data-ng-repeat="productVersion in filteredProductVersions = (productVersions | deploymentNameOnlyQuery: productVersionQuery | otherPublishedOrDeprecatedProductVersions: productVersion)" ng-class-even="'bxTableRowEven'" ng-class-odd="'bxTableRowOdd'" ng-class="'productRow'">
  ...
  <div data-ng-switch-when="2">
    <h5 style="margin-bottom: 15px;">{{::'product_manage_replace_plans' | translate}}</h5>
    <div class="tableHeaderBackground" style="padding-left: 10px;">
      <div class="column halfColumn">
        <div>{{::'product_manage_replace_action' | translate}}</div>
        <div data-ng-if="!selectedProductVersion.document.info">{{::'loading' | translate}}</div>
        <h3 style="margin-bottom: 10px;">{{selectedProductVersion.document.info.title}}&nbsp;&nbsp;{{selectedProductVersion.document.info.version}}</h3>
      </div>
      <div class="column halfColumn">
        <div>{{::'product_replace_with' | translate}}</div>
        <div data-ng-if="!productVersion.document.info">{{::'loading' | translate}}</div>
    ...
    <form name="$parent.productMappings" class="bx--form">
      <ul class="simpleList">
        <li class="secondaryBorder borderBottom" style="padding: 10px;" data-ng-repeat="plan in selectedProductVersion.dependents.PLANVERSION">
          <div class="bx--select labeledSelect">
            <label class="bx--form__label">{{plan.planDisplayName}}</label>
            <select class="productPlanSelector bx--select__input" required data-ng-model="planTargets[plan.planName]" title="{{::'productReplaceSelectTarget' | translate}}">
              <option data-ng-repeat="otherPlan in productVersion.dependents.PLANVERSION" value="{{otherPlan.planName}}">{{otherPlan.planDisplayName}}</option>
            </select>
...
<div class="bx--modal__buttons" role="contentinfo" aria-label="{{::'footerSectionLabel' | translate}}">
  ...
  <div class="bx--modal__buttons-container">
    ...
    <button class="bx--btn no-animate" type="button" data-ng-show="currentPage < 2" data-ng-disabled="!selectedProductVersion" data-ng-click="currentPage = 2">
      {{::'product_lifecycle_next' | translate}}
    </button>
    <button class="bx--btn no-animate" type="button" data-ng-show="currentPage == 2" data-ng-disabled="!productMappings.$valid" data-ng-click="replaceVersion(selectedProductVersion)">
      {{::'product_manage_replace_action' | translate}}
    </button>
  </div>
</div>
