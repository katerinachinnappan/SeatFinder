...
test('Test apic extensions:pull command', function(t) {
  ...
  // no config
  t.test('extensions:pull ' + nameVersion + ' --server ' + server + ' --organization ' + org + ' --catalog ' + catalog + ' # No config', function(t) {
  ...
  t.test('extensions:pull ' + nameVersion + ' -s ' + server + ' -o ' + org
    + ' -c ' + catalog + ' # No config', function(t) {
    return apic('extensions:pull ' + nameVersion + ' -s ' + server + ' -o ' + org + ' -c ' + catalog)
      .then(v.bind(null, t, [ 'Pulled ' + nameVersion + ' extension from ' + org + ':' + catalog + ' [' + fileName + ']' ]))
      .then(clear);
  });

  // with config
  t.test('extensions:pull ' + nameVersion + ' --server ' + server + ' # config', function(t) {
    return setCatalog()
      .then(setOrg)
      .then(logger.clear)
      .then(apic.bind(null, 'extensions:pull ' + nameVersion + ' --server ' + server))
      .then(v.bind(null, t, [ 'Pulled ' + nameVersion + ' extension from ' + org + ':' + catalog + ' [' + fileName + ']' ]))
      .then(clear);
  });
