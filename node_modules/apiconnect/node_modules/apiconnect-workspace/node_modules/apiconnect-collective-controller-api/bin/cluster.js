...
var commands = {
  listClusterNames: require('../lib/cluster').listClusterNames,
  listMembers: require('../lib/cluster').listMembers,
  restartCluster: require('../lib/cluster').restartCluster,
  stopCluster: require('../lib/cluster').stopCluster,
  startCluster: require('../lib/cluster').startCluster,
  version: version,
};
...
while ((option = serverCommandParser.getopt()) !== undefined) {
  switch (option.option) {
    ...
    case 'w': args.wlpnUserDir = option.optarg; break;
    case 'r': args.rpcUser = option.optarg; break;
    case 'o': args.rpcPassword = option.optarg; break;
...
function printHelp($0, prn) {
  var USAGE = fs.readFileSync(require.resolve('./cluster.txt'), 'utf-8')
    .replace(/%MAIN%/g, $0)
    .trim();
  prn(USAGE);
}
